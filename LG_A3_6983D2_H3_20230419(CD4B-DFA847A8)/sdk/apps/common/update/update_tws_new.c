...
int tws_ota_data_send_m_to_s(u8 *buf, u16 len)
{
    ...
    while (retry --) {
        ret = tws_ota_trans_to_sibling(data, len + 1);
        if (ret < 0 && ret != -1) {
            os_time_dly(5);
        } else {
            break;
...
{
    u8 rsp_data = OTA_TWS_WRITE_BOOT_INFO;
    *up_flg = 1;
    if (tws_api_get_tws_state() & TWS_STA_SIBLING_DISCONNECTED) {
...
{
    int ret = 0;
    u8 *data = malloc(sizeof(struct __tws_ota_para) + 1);
    if (!data) {
        ret = -1;
        goto _ERR_RET;
    }
    ...
    memcpy(data + 1, para, sizeof(struct __tws_ota_para));
    log_info("tws_ota_open2\n");
    if (tws_ota_trans_to_sibling(data, sizeof(struct __tws_ota_para) + 1)) {
        ret = -1;
        goto _ERR_RET;
    }
    if (os_sem_pend(&tws_ota_sem, 300) == OS_TIMEOUT) {
...
static void deal_sibling_tws_ota_trans(void *data, u16 len, bool rx)
{
    ...
    if (rx) {
        ...
        case OTA_TWS_START_UPDATE:
            ...
            memcpy(&para, recv_data + 1, sizeof(struct __tws_ota_para));
        ...
        case OTA_TWS_TRANS_UPDATE_DATA:
            printf("MSG_OTA_TWS_TRANS_UPDATE_DATA\n");
            dual_bank_update_write(recv_data + 1, len - 1, tws_trans_data_write_callback);
            break;
        case OTA_TWS_TRANS_UPDATE_DATA_RSP:
            printf("MSG_OTA_TWS_TRANS_UPDATE_DATA_RSP\n");
            if (recv_data[1] == OTA_TWS_CMD_SUCC) {
                os_sem_post(&tws_ota_sem);
            }
            break;
        case OTA_TWS_VERIFY:
            ...
            ret =  dual_bank_update_verify(NULL, NULL, tws_verify_result_hdl);
            if (ret) {
                rsp_data[0] = OTA_TWS_VERIFY_RSP;
                rsp_data[1] = OTA_TWS_CMD_ERR;
                tws_ota_trans_to_sibling(rsp_data, 2);
        ...
        case OTA_TWS_VERIFY_RSP:
            g_printf("MSG_OTA_TWS_VERIFY_RSP\n");
            if (recv_data[1] == OTA_TWS_CMD_SUCC) {
                os_sem_post(&tws_ota_sem);
            }
            break;
        ...
        case OTA_TWS_USER_CHIP_UPDATE:
            if (user_chip_tws_update_handle) {
                user_chip_tws_update_handle(recv_data + 1, len - 1);
            }
            break;
        }
    }
